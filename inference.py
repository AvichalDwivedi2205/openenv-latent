...
def _resolve_token(api_base_url: str, cli_token: str | None) -> str | None:
    """Pick the credential that matches the active OpenAI-compatible endpoint."""
    if cli_token:
        return cli_token
...
def _resolve_submission_env(cli_model: str | None, cli_api_base_url: str | None, cli_token: str | None) -> tuple[str, str]:
    ...
    model_name = cli_model or os.getenv("MODEL_NAME") or os.getenv("BASELINE_MODEL") or DEFAULT_MODEL_NAME
    ...
    if not token:
        raise ValueError(
            ...
            "For local Gradient runs, you can also set DIGITALOCEAN_API_TOKEN or pass --token."
        )
