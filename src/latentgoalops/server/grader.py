...
from latentgoalops.server.rewards import cosine_similarity, strategy_embedding
...
def trajectory_coherence(actions: list[Any], split_index: int | None = None) -> float:
    ...
    if split_index is None or split_index <= 0 or split_index >= len(actions):
        ...
        for previous, current in pairs:
            similarities.append(
                max(0.0, cosine_similarity(strategy_embedding(previous), strategy_embedding(current)))
            )
    else:
