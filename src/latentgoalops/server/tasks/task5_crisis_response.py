...
def _focus_account(accounts: list[CustomerAccount], goal_name: str) -> CustomerAccount:
    if goal_name == "growth":
        return max(accounts, key=lambda account: (account.segment in {"self_serve", "smb"}, account.champion_strength))
    if goal_name == "retention":
        return min(accounts, key=lambda account: (account.renewal_window_days, -account.strategic_importance))
    if goal_name == "revenue":
        return max(accounts, key=lambda account: (account.expansion_potential, account.annual_contract_value))
    return max(accounts, key=lambda account: (account.support_tier == "premium", account.integration_complexity))


def _build_inbox(goal_name: str, accounts: list[CustomerAccount], market_context, dashboard: DashboardState) -> list[InboxItem]:
    hot_accounts = sorted(
        accounts,
        key=lambda account: (
    ...
    focus_account = _focus_account(accounts, goal_name)
    items = [
        InboxItem(
            item_id="crisis_msg_0",
            sender="CEO",
            text=f"CEO note: {signal_lines[0]} {signal_lines[1]}",
        ...
        InboxItem(
            ...
            sender="CFO",
            text=(
                f"CFO note: runway is {market_context.cash_runway_months} months, board pressure is "
                f"{market_context.board_pressure_level:.2f}, and the package needs a defendable downside profile."
            ),
...
def _narrative(goal_name: str, accounts: list[CustomerAccount], market_context, dashboard: DashboardState) -> str:
...
def evaluate_task5_action_value(
    ...
    value = _initiative_bundle_value(chosen_items, weights)
...
def build_task5_episode(
    ...
    goal_name = hidden_goal.archetype.value
    narrative = _narrative(goal_name, accounts, market_context, dashboard)
    inbox = _build_inbox(goal_name, accounts, market_context, dashboard)
    episode = {
